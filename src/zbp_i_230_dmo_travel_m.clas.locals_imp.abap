CLASS lsc_zi_230_dmo_travel_m DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zi_230_dmo_travel_m IMPLEMENTATION.
...
********************************************************************************
    ...
    " (1) Get instance data of all instances that have been created
    IF create-booking_supplement IS NOT INITIAL.
      booksuppls_db = CORRESPONDING #( create-booking_supplement MAPPING FROM ENTITY ).
    ...
    " (2) Get instance data of all instances that have been updated during the transaction
    booksuppls_db = CORRESPONDING #( update-booking_supplement MAPPING FROM ENTITY ).
    IF booksuppls_db IS NOT INITIAL.
      ...
      LOOP AT update-booking_supplement ASSIGNING FIELD-SYMBOL(<unmanaged_booksuppl>).
    ...
    " (3) Get keys of all travel instances that have been deleted during the transaction
    IF delete-booking_supplement IS NOT INITIAL.
      booksuppls_db = CORRESPONDING #( delete-booking_supplement MAPPING FROM ENTITY ).
...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS accept_travel FOR MODIFY
      IMPORTING keys FOR ACTION travel~accept_travel RESULT result.
    ...
    METHODS recalc_total_price FOR MODIFY
      IMPORTING keys FOR ACTION travel~recalc_total_price.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>) WHERE travel_id IS NOT INITIAL.
      APPEND CORRESPONDING #( <entity> ) TO mapped-travel.
    ENDLOOP.

    DATA(entities_initial_travel_id) = entities.
    DELETE entities_initial_travel_id WHERE travel_id IS NOT INITIAL.
    CHECK entities_initial_travel_id IS NOT INITIAL.
    ...
    TRY.
        cl_numberrange_runtime=>number_get(
          EXPORTING
            nr_range_nr       = '01'
            object            = '/DMO/TRV_M'
            quantity          = CONV #( lines( entities_initial_travel_id ) )
          IMPORTING
      ...
      CATCH cx_number_ranges INTO DATA(lx_number_ranges).
        LOOP AT entities_initial_travel_id ASSIGNING <entity>.
          APPEND VALUE #(
              %cid = <entity>-%cid
              %key = <entity>-%key
    ...
    DATA(travel_id_max) = number_range_key.
    travel_id_max -= number_range_returned_quantity.
  ...
  METHOD copy_travel.
    ...
    DATA:
      travels       TYPE TABLE FOR CREATE zi_230_dmo_travel_m\\travel,
      bookings_cba  TYPE TABLE FOR CREATE zi_230_dmo_travel_m\\travel\_booking,
      booksuppl_cba TYPE TABLE FOR CREATE zi_230_dmo_travel_m\\booking\_booking_supplement.
    ...
    ENTITY travel
      ...
      CREATE BY \_booking FIELDS ( booking_id booking_date customer_id carrier_id connection_id flight_date flight_price currency_code booking_status )
        WITH bookings_cba
    ENTITY booking
      CREATE BY \_booking_supplement FIELDS ( booking_supplement_id supplement_id price currency_code )
        WITH booksuppl_cba
  ...
  METHOD get_instance_features.
    ...
    result = VALUE #( FOR travel IN travels
                       ( %tky                            = travel-%tky
                         %features-%action-accept_travel = COND #( WHEN travel-overall_status = 'A'
